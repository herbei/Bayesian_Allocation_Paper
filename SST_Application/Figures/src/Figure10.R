...
# Figure 10: thresholded posterior inclusion probabilities for X and beta
...
resolve_project_root <- function() {
  ...
  for (cand in candidates) {
    if (file.exists(file.path(cand, "modules", "plot_utils.R")) &&
        dir.exists(file.path(cand, "Figures"))) {
      return(cand)
    }
  }
  stop("Could not locate project root containing modules/plot_utils.R and Figures.")
}
...
best_rdata <- resolve_figure_chain_file(figure_root, "SST_FIGURE10_CHAIN_FILE")
...
if (is.null(B_chain) || length(dim(B_chain)) != 2L || ncol(B_chain) == 0L) {
  stop("No B-chain columns available for PIP diagnostics in best-run artifact.")
}
if (length(keep_idx) == 0L) {
  stop("No kept post-burn draws available in best-run artifact.")
}
...
build_pip_heatmap <- function(df, title_text = NULL, show_legend = TRUE, breaks = NULL, labels = NULL) {
  ...
  p <- ggplot(df, aes(x = k, y = component, fill = prob_display)) +
    ggplot2::geom_tile(width = 1, height = 0.82) +
    x_scale +
...
if (is.null(legend_grob)) {
  stop("Could not extract legend for Figure 10.")
}
...
output_pdf <- file.path(fig_dir, "Figure10.pdf")
output_png <- file.path(fig_dir, "Figure10.png")

figure_width_in <- 7.4
figure_height_in <- 4.25
...
cropped_pdf <- tempfile(pattern = "Figure10_crop_", tmpdir = fig_dir, fileext = ".pdf")
...
message("Best-run file: ", normalizePath(best_rdata, mustWork = FALSE))
